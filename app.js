...
const subEl = qs('#cart-sub'); const discEl = qs('#cart-disc'); const shipEl = qs('#cart-ship'); const totalEl = qs('#cart-total');
const ship = 10000;
...
function renderCart(){
  ...
  items.forEach(it=>{
    ...
    `;
    const [minus, input, plus] = qsa('.qty *', row);
    minus.addEventListener('click', ()=> updateQty(it.id, (+input.value||1)-1));
    plus.addEventListener('click', ()=> updateQty(it.id, (+input.value||1)+1));
    input.addEventListener('change', ()=> updateQty(it.id, (+input.value||1)));
    qsa('button', row).pop().addEventListener('click', ()=> removeFromCart(it.id));
    cartItems.appendChild(row);
  });
  const sub = items.reduce((a,b)=> a+b.price*b.qty,0);
  const disc = discount(items);
  subEl.textContent = fmtIDR(sub);
  discEl.textContent = disc?('-'+fmtIDR(disc)):'-';
  totalEl.textContent = fmtIDR(sub-disc+(items.length?ship:0));
}
...
function removeFromCart(id){
  writeCart(readCart().filter(i=>i.id!==id)); renderCart();
}
...
const closeCo = qs('#close-checkout'); const form = qs('#checkout-form'); const done = qs('#checkout-done');
...
closeCo.addEventListener('click', ()=> modal.hidden=true);
form.addEventListener('submit', (e)=>{
  e.preventDefault();
  done.hidden=false; form.hidden=true;
  localStorage.setItem(CART_KEY,'[]'); renderCart();
});
qs('#finish')?.addEventListener('click', ()=>{ done.hidden=true; form.hidden=false; form.reset(); modal.hidden=true; });

// Testimoni (localStorage)
const TKEY='cu_testi_v1';
